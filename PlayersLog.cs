...
public class PlayersLog : BasePlugin
{
    ...
    public HookResult OnPlayerConnect(EventPlayerConnect @event, GameEventInfo info)
    {
        ...
        //isAdmin ? $"Admin {playerName} connected, data:[{data}], time:[{time}]" : 
        var text =$"{playerName} connected, data:[{data}], time:[{time}]";
        if (!File.Exists(path))
    ...
    public HookResult OnPlayerDisconnect(EventPlayerDisconnect @event, GameEventInfo info)
    {
        ...
        var time = DateTime.Now.ToString("HH:mm:ss");
        var path = "PlayersLog.txt";
        //isAdmin ? $"Admin {playerName} disconnected, data:[{data}], time:[{time}]" :
        var text = $"{playerName} disconnected, data:[{data}], time:[{time}]";
        if (!File.Exists(path))
            File.Create(path).Close();
        File.AppendAllText(path, text + Environment.NewLine);
        return HookResult.Continue;
